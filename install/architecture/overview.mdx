...
This page focuses on describing the main components of OpSyn and focus mainly on workflow executions.
...
**Third Party**:
- **Postgres**: The main database for OpSyn.
...
- **Webhooks**:  
  All webhooks are sent to the OpSyn app, which performs basic validation and adds them to the queue. In case of a spike, webhooks will be added to the queue.
...
To scale OpSyn, you typically need to increase the replicas of either workers, the app, or the Postgres database. A small Redis instance is sufficient as it can handle thousands of jobs per second and rarely acts as a bottleneck.
