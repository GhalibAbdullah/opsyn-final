...
On average, OpSyn (self-hosted) can handle 95 flow executions per second on a single instance (including PostgreSQL and Redis) with under 300ms latency.\
...
1. Run OpSyn with PostgreSQL and Redis with the following environment variables:
...
<Tip>
  These benchmarks are based on running OpSyn in `SANDBOX_CODE_ONLY` mode. This does **not** represent the performance of OpSyn Cloud, which uses a different sandboxing mechanism to support multi-tenancy. For more information, see [Sandboxing](/install/architecture/workers#sandboxing).
</Tip>