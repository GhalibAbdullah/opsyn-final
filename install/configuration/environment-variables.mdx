...
| `AP_CLOUD_AUTH_ENABLED`            | Turn off the utilization of built-in OAuth2 applications                                                                                                                                                         | `false`                                                                        |                                                                        |
| `AP_DB_TYPE`                       | The type of database to use. (POSTGRES / SQLITE3)                                                                                                                                                                    | `SQLITE3`                                                                      |                                                                        |
| `AP_EXECUTION_MODE`                | You can choose between 'SANDBOX_PROCESS', 'UNSANDBOXED', 'SANDBOX_CODE_ONLY', 'SANDBOX_CODE_AND_PROCESS' as possible values. If you decide to change this, make sure to carefully read the [Workers documentation](/install/architecture/workers)                                                                | `UNSANDBOXED`                                                                  |                                                                        |
...
| `AP_INTERNAL_URL`                  | (BETA) Used to specify the SSO authentication URL.                                                                                 | None                                                                         | |
| `AP_JWT_SECRET`                    | ❗️ Encryption key used for generating JWT tokens is a 32-character hexadecimal key. You can generate one using the following command: `openssl rand -hex 32`.                                                                                                                                                                     | None                                                                         | |
...
| `AP_TEMPLATES_SOURCE_URL`          | This is the endpoint we query for templates, remove it and templates will be removed from UI                                                                                                                         | `https://cloud.activepieces.com/api/v1/flow-templates` (upstream default)                         |                                                                        |
...
| `SMTP_SENDER_EMAIL`                | The email address from which activepieces sends emails.                                                                                                                                                              | `None`                                                                         |  test@mail.example.com                                                 |
...
| `AP_APP_TITLE` | Initial title shown in the browser tab while loading the app | `OpSyn`  | `OpSyn`
...
OpSyn supports both standard OpenTelemetry environment variables and vendor-specific configuration for observability and tracing.
...
OpSyn will only use the configuration from the environment variables if `AP_SMTP_HOST`, `AP_SMTP_PORT`, `AP_SMTP_USERNAME` and `AP_SMTP_PASSWORD` all have a value set. TLS is supported.
