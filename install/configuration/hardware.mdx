...
icon: "server"
description: "Specifications for hosting OpSyn"
---
...
OpSyn is designed to be memory-intensive rather than CPU-intensive. A modest instance will suffice for most scenarios, but requirements can vary based on specific use cases.
...
Redis requires minimal scaling as it primarily stores jobs during processing. OpSyn leverages BullMQ, capable of handling a substantial number of jobs per second.
...
<Tip>
**Scaling Tip:** The OpSyn container is stateless, allowing for seamless horizontal scaling.
</Tip>
...
OpSyn ensures no request is lost; all requests are queued. In the event of a spike, requests will be processed later, which is acceptable as most flows are asynchronous, with synchronous flows being prioritized.
