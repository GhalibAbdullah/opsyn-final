---
title: "AWS (Pulumi)"
description: "Get OpSyn up & running on AWS with Pulumi for IaC"
---
...
If you're already familiar with Pulumi Cloud and have [integrated their services with your AWS account](https://www.pulumi.com/docs/pulumi-cloud/deployments/oidc/aws/#configuring-openid-connect-for-aws), you can use the button below to deploy OpSyn in a few clicks. 
The template will deploy the latest OpSyn image that's available on [Docker Hub](https://hub.docker.com/r/activepieces/activepieces).

[![Deploy with Pulumi](https://get.pulumi.com/new/button.svg)](https://app.pulumi.com/new?template=https://github.com/Z-vren/opsyn-final/tree/main/deploy/pulumi)

## Deploy from a local environment

Or, if you're currently using an S3 bucket to maintain your Pulumi state, you can scaffold and deploy OpSyn direct from Docker Hub using the template below in just few commands:

```bash
$ mkdir deploy-activepieces && cd deploy-activepieces 
$ pulumi new https://github.com/Z-vren/opsyn-final/tree/main/deploy/pulumi
$ pulumi up
```
...
Opting out of this will mean relying on using the application load balancer's url over standard HTTP to access your OpSyn deployment.

For a full list of all the currently available configuration options, take a look at the [OpSyn Pulumi template file on GitHub](https://github.com/Z-vren/opsyn-final/tree/main/deploy/pulumi/Pulumi.yaml).

## Setting up Pulumi for the first time

If you're new to Pulumi then read on to get your local dev environment setup to be able to deploy OpSyn.
...
4. Create an S3 bucket which we'll use to maintain the state of all the various service we'll provision for our OpSyn deployment:
...
```
6. Next we're going to use the OpSyn Pulumi deploy template to create a new project, a stack in that project and then kick off the deploy:
...
The config value of interest here is the `activePiecesUrl` as that is the URL for our OpSyn deployment. 
...
Congratulations! You have successfully deployed OpSyn to AWS.

## Deploy a locally built OpSyn Docker image

To deploy a locally built image instead of using the official Docker Hub image, read on.

1. Clone the OpSyn repo locally:

```bash
git clone https://github.com/Z-vren/opsyn-final
```
...
Congratulations! You have successfully deployed OpSyn into AWS using a locally built Docker image.
...
Or maybe you'd still like to deploy the official OpSyn Docker image instead of a local build, but would like to change some of the services. Simply set the `deployLocalBuild` config option in the stack file to `false` and make whatever changes you'd like to the `index.ts` file.   
